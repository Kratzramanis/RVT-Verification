...
Original file is located at
    https://colab.research.google.com/drive/1iUBONE2SmdWBUZBQaPUS1HkTorjPl3Rr
"""
...
funnel_normal, _, _ = opposing_funnel_2d(
    ...
    amp_891_normal, amp_M101_normal, width_891, width_M101, length_scale, target_angle=target_angle
)

funnel_incl, _, _ = opposing_funnel_2d(
    X2, Y2, center_891_2d, center_M101_2d,
    amp_891_incl, amp_M101_incl, width_891, width_M101, length_scale, target_angle=target_angle
)
...
# rcount/ccount schützen Colab vor dem Einfrieren bei N=140
...
# Winkel perfekt auf die beiden Trichteröffnungen ausrichten
...
# ==============================================================================
# ERWEITERUNGEN FÜR RVT/AVT: NEGENTROPIE UND 2D-PROJEKTION (KORRIGIERT)
# ==============================================================================

# 1. Negentropie-Vektor (Anti-Entropie)
S_vector = -gradient_3d(V_12_base, dx)  # Räumlicher Gradient als Näherung für ∂Φ/∂t
S_magnitude = np.linalg.norm(S_vector, axis=-1)

# 2. 2D-Projektion auf den Lorentzschen Äther (KORRIGIERT)
# RVT-Korrekturtensor E_mu_nu (vereinfacht für 3D)
E_xx = (np.roll(V_12_base, 1, axis=0) + np.roll(V_12_base, -1, axis=0) - 2 * V_12_base) / (dx**2)
E_yy = (np.roll(V_12_base, 1, axis=1) + np.roll(V_12_base, -1, axis=1) - 2 * V_12_base) / (dx**2)
E_zz = (np.roll(V_12_base, 1, axis=2) + np.roll(V_12_base, -1, axis=2) - 2 * V_12_base) / (dx**2)

# 2D-Projektionsspannung: S_2D = E_xx + E_yy (Spur in der x-y-Ebene)
S_2D = E_xx + E_yy

# 3. Bianchi-Konsistenz (vereinfacht)
div_E = np.gradient(E_xx + E_yy + E_zz, dx, dx, dx)
print(f"Bianchi-Konsistenz: max(|∇·E|) = {np.max(np.abs(div_E)):.2e} (sollte ≈ 0 sein)")
...
# ==============================================================================
# RVT/AVT: 3D-zu-2D-Projektion mit Kopplungsresonanz, Negentropie und Bianchi-Konsistenz
# ==============================================================================
...
import warnings
warnings.filterwarnings("ignore")  # Unterdrückt UserWarnings für Colorbar

# --- Parameter aus deiner Trichter-Zelle ---
N_anim = 40  # Gitterauflösung
L = 180.0    # Systemgröße in kpc
dx = L / (N_anim - 1)
C_RVT = 0.8  # RVT-Kopplungskonstante

# Galaxienparameter (NGC 891 und M101)
pos_891 = np.array([-35.0, 0.0, 0.0])
pos_M101 = np.array([35.0, 0.0, 0.0])
scale_891 = 10.0
scale_M101 = 15.0
...
# ==============================================================================
# ERWEITERUNGEN FÜR RVT/AVT: NEGENTROPIE, 2D-PROJEKTION, BIANCHI-KONSISTENZ
# ==============================================================================

# 1. Negentropie-Vektor (Anti-Entropie)
S_vector = -gradient_3d(V_12_base, dx)  # Näherung für ∂Φ/∂t
S_magnitude = np.linalg.norm(S_vector, axis=-1)

# 2. Korrekte Berechnung von E_mu_nu aus Xi (nicht aus V_12!)
laplace_Xi = (np.roll(Xi, 1, axis=0) + np.roll(Xi, -1, axis=0) +
              np.roll(Xi, 1, axis=1) + np.roll(Xi, -1, axis=1) +
              np.roll(Xi, 1, axis=2) + np.roll(Xi, -1, axis=2) -
              6 * Xi) / (dx**2)

# E_mu_nu = ∇_μ ∇_ν Xi - g_μν □Xi (im flachen Raum: g_μν = diag(1,1,1))
E_xx = (np.roll(Xi, 1, axis=0) + np.roll(Xi, -1, axis=0) - 2 * Xi) / (dx**2) - laplace_Xi
E_yy = (np.roll(Xi, 1, axis=1) + np.roll(Xi, -1, axis=1) - 2 * Xi) / (dx**2) - laplace_Xi
E_zz = (np.roll(Xi, 1, axis=2) + np.roll(Xi, -1, axis=2) - 2 * Xi) / (dx**2) - laplace_Xi
...
# ==============================================================================
# ANIMATION: 3D-GEFÜGE + 2D-PROJEKTION (OHNE FFMPEG)
# ==============================================================================

# Zeitstauchung (1 Bogenminute = 10s Video)
total_duration_seconds = 10.0
frames = 60
frame_interval_ms = (total_duration_seconds * 1000) / frames
...
# Projizierte Positionen der Galaxien (für 2D-Plot)
x_proj_891 = pos_891[0] * np.cos(np.radians(30))
y_proj_891 = pos_891[0] * np.sin(np.radians(30))
x_proj_M101 = pos_M101[0] * np.cos(np.radians(-20))
y_proj_M101 = pos_M101[0] * np.sin(np.radians(-20))
...
def update(frame):
    ...
    # Raum-zeitliche Verformung der Kopplungsachse
    y_wave = 25.0 * np.sin(np.pi * t_bruecke) * np.cos(tau_phase) * (K_res_M101 / K_res_891)
    z_wave = 15.0 * np.sin(np.pi * t_bruecke) * np.sin(tau_phase) * (K_res_891 / K_res_M101)
    ...
    # Zentren markieren
    ax_3d.scatter(pos_891[0], pos_891[1], pos_891[2], color='red', s=180, marker='x', linewidth=3, label='NGC 891 Zentrum')
    ax_3d.scatter(pos_M101[0], pos_M101[1], pos_M101[2], color='blue', s=180, marker='x', linewidth=3, label='M101 Zentrum')

    # Kopplungsachse
    ax_3d.plot(x_bruecke, y_bruecke, z_bruecke, color='cyan', linewidth=4, label=r'Resonanzachse $\Xi_{12}(\vec{\tau})$', zorder=5)
    ...
    try:
        ...
        if threshold > 0:
            ...
            verts_scaled = verts - [L/2, L/2, L/2]
            mesh = Poly3DCollection(verts_scaled[faces], alpha=0.2, cmap='coolwarm')
            mesh.set_clim(vmin=np.min(V_12_dynamic), vmax=np.max(V_12_dynamic))
            ax_3d.add_collection3d(mesh)
    except (ValueError, RuntimeError):
        pass

    # Negentropie-Vektor (Quiver-Plot)
    ax_3d.quiver(X[::5, ::5, ::5], Y[::5, ::5, ::5], Z[::5, ::5, ::5],
                 S_vector[::5, ::5, ::5, 0], S_vector[::5, ::5, ::5, 1], S_vector[::5, ::5, ::5, 2],
                 length=2.0, normalize=True, color='magenta', label=r'Negentropie $\vec{S}$')

    # 3D-Achsenbeschriftung
    ...
    ax_3d.set_title(r'3D: RVT-Bogenminute mit Trichter-Kopplungsresonanz & Negentropie')
    ax_3d.legend(loc='upper left')
    ...
    z_slice = N_anim // 2
    S_2D_slice = S_2D[:, :, z_slice]
    contour_2d = ax_2d.contourf(X[:, :, z_slice], Y[:, :, z_slice], S_2D_slice,
...
# Animation erstellen (ohne ffmpeg)
ani = FuncAnimation(fig, update, frames=frames, interval=frame_interval_ms, blit=False)

# HTML5-Ausgabe (für Jupyter Notebook)
display(HTML(ani.to_html5_video()))

# Optional: Als GIF speichern (falls gewünscht)
# ani.save('RVT_AVT_Animation.gif', writer=PillowWriter(fps=10))

# ==============================================================================
# RVT/AVT: 3D-zu-2D-Projektion mit Kopplungsresonanz, Negentropie und Bianchi-Konsistenz
# KORRIGIERTE VERSION MIT GLÄTTUNG UND NORMIERUNG
# ==============================================================================
...
# Metrik im flachen Raum: g_μν = diag(-1, 1, 1, 1) → für 3D: g_μν = diag(1, 1, 1)
laplace_Xi = (np.roll(Xi, 1, axis=0) + np.roll(Xi, -1, axis=0) +
              np.roll(Xi, 1, axis=1) + np.roll(Xi, -1, axis=1) +
              np.roll(Xi, 1, axis=2) + np.roll(Xi, -1, axis=2) -
              6 * Xi) / (dx**2)

# E_mu_nu = ∇_μ ∇_ν Xi - g_μν □Xi
...
# ==============================================================================
# ANIMATION: 3D-GEFÜGE + 2D-PROJEKTION
# ==============================================================================
...
def update(frame):
    ...
    y_bruecke = pos_891[1] * L + (pos_M101[1] * L - pos_891[1] * L) * t_bruecke + y_wave
    z_bruecke = pos_891[2] * L + (pos_M101[2] * L - pos_891[2] * L) * t_bruecke + z_wave
    ...
    try:
        threshold = 0.15 * np.max(np.abs(V_12_dynamic))
        if threshold > 0:
            verts, faces, _, _ = marching_cubes(V_12_dynamic, threshold, spacing=(dx*L, dx*L, dx*L))
            verts_scaled = verts - [L/2, L/2, L/2]
            mesh = Poly3DCollection(verts_scaled[faces], alpha=0.2, cmap='coolwarm')
            mesh.set_clim(vmin=np.min(V_12_dynamic), vmax=np.max(V_12_dynamic))
            ax_3d.add_collection3d(mesh)
    ...
    z_slice = N_anim // 2
    S_2D_slice = S_2D[:, :, z_slice]
    contour_2d = ax_2d.contourf(X[:, :, z_slice]*L, Y[:, :, z_slice]*L, S_2D_slice,
                               levels=20, cmap='plasma', alpha=0.7)
    fig.colorbar(contour_2d, ax=ax_2d, label=r'$\mathcal{S}_{2D}$')
    ax_2d.scatter(x_proj_891, y_proj_891, color='red', s=200, label='NGC 891 (projiziert)')
    ax_2d.scatter(x_proj_M101, y_proj_M101, color='blue', s=200, label='M101 (projiziert)')
